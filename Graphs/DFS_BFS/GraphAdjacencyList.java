...
public class GraphAdjacencyList {
    ...
    public void BFS(GraphAdjacencyList g, vertex v) {
        ...
        System.out.println("Vertex visited: " + v.name);

        while (!queue.isEmpty()) {
            vertex vertexDequeued = queue.removeFirst();

            for (vertex neighbour : vertexDequeued.friendsList) {
                if (!neighbour.visited) {
                    queue.add(neighbour);
                    neighbour.visited = true;
                    System.out.println("Vertex visited: " + neighbour.name);
